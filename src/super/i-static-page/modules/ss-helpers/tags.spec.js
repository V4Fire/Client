...
`);
	...
	describe('`getStyleDecl`', () => {
		describe('providing `src`', () => {
			...
			it('with inlining', async () => {
				const
					decl = await ss.getStyleDecl({src: 'src/base/b-bottom-slide/b-bottom-slide.styl', inline: true});

				expect(decl).toBe("<style >include('src/base/b-bottom-slide/b-bottom-slide.styl');</style>");
			});
...
`);
			...
			it('with JS initializing and inlining', async () => {
				const decl = collapseSpaces(
					await ss.getStyleDecl({src: 'src/base/b-bottom-slide/b-bottom-slide.styl', js: true, inline: true})
				);

				expect(decl.trim()).toBe(`(function () {
	var el = document.createElement('style');
//#set convertToStringLiteral
el.innerHTML = include('src/base/b-bottom-slide/b-bottom-slide.styl');
