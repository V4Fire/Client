...
module.exports = (page, {browser, contextOpts}) => {
	...
	describe('`core/session`', () => {
		...
		describe('get', () => {
			it('returns session data if session was initialized', async () => {
				await session.evaluate((ctx) => ctx.set('authToken', {someParam: 1}));
			...
			it('returns undefined if session was not initialized', async () => {
				const
					testVal = await session.evaluate((ctx) => ctx.get());
		...
		describe('isExists', () => {
			it('returns `true` is session does exists', async () => {
				await session.evaluate((ctx) => ctx.set('authToken', {someParam: 1}));
			...
			it('returns `false` is session does not exists', async () => {
				const
					testVal = await session.evaluate((ctx) => ctx.isExists());
