...
test.describe('core/kv-storage/engines/cookie', () => {
	...
	test.beforeEach(async () => {
		await cookiesAPI.evaluate((cookies, [cookieName]) => cookies.remove(cookieName), [cookieName]);
	});

	test.describe('`has`', () => {
		test.beforeEach(async () => {
			await cookiesAPI.evaluate((cookies, [name, data]) => cookies.set(name, data), [cookieName, fixture]);
		});
	...
	test.describe('`set`', () => {
		test('should set cookie values based on the provided keys', async () => {
			let cookieValue = await cookiesAPI.evaluate((cookies, [name]) => cookies.get(name), [cookieName]);
			...
			cookieValue = await cookiesAPI.evaluate((cookies, [name]) => cookies.get(name), [cookieName]);
