...
test.describe('core/component/event', () => {
	...
	test('добавление обработчика через метод $on должно происходить через $async', async () => {
	...
	test('добавление обработчика через метод $once должно происходить через $async', async () => {
		const isWrapped = await target.evaluate((ctx) => {
