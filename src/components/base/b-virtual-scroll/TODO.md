...
- reset.load - перезагрузит ли компонент? написать тест кейсы на перезагрузки с помощью reload, изменения request, вызова reset.load
- описать в документации про тестовый компонент в демо странице и для чего он нужен
- сделать хэндлеры и модули протектед
