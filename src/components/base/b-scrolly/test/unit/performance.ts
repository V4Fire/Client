...
test.describe('b-scrolly rendering', () => {
	...
	// 'updateListWithShallowRef'
	// ['updateListWithShallowRef'].forEach((method) => {

	['updateListWithShallowRef', 'pushToListCreateElement'].forEach((method) => {
		test(`b-scrolly perf ${method}`, async ({page, browser, context}) => {
			...
			await scrolly.evaluate((c) => c[globalThis.method](50));
