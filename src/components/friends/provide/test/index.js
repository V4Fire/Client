...
module.exports = async (page, params) => {
	...
	describe('`iBlock.provide`', () => {
		...
		describe('`fullElementName`', () => {
			it('simple usage', async () => {
				expect(await target.evaluate(({provide}) => provide.fullElementName('foo')))
					.toBe('b-dummy__foo');
			});

			it('providing a modifier', async () => {
				expect(await target.evaluate(({provide}) => provide.fullElementName('foo', 'opened', true)))
					.toBe('b-dummy__foo_opened_true');
			});

			it('providing a component name', async () => {
				expect(await target.evaluate(({provide}) => provide.fullElementName('b-foo', 'foo')))
					.toBe('b-foo__foo');
			});

			it('providing a component name and modifiers', async () => {
				expect(await target.evaluate(({provide}) => provide.fullElementName('b-foo', 'foo', 'opened', true)))
		...
		describe('`elClasses`', () => {
			it('simple usage', async () => {
				...
				expect(await target.evaluate(({provide}) => provide.elementClasses({button: {focused: true}})))
					.toEqual([id, 'b-dummy__button', 'b-dummy__button_focused_true']);
			});

			it('providing a component name', async () => {
				expect(await target.evaluate(({provide}) => provide.elementClasses('b-foo', {button: {focused: true}})))
			...
			it('providing a component context', async () => {
				...
				expect(await target.evaluate(({provide}, ctx) => provide.elementClasses(ctx, {button: {focused: true}}), ctx))
