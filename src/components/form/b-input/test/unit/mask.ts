...
test.describe('<b-input> mask API', () => {
	...
	test('checking `value` getter with empty input', async ({page}) => {
		const target = await renderInput(page, {});
		test.expect(await target.evaluate((ctx) => ctx.value)).toBe('');
	});

	test('checking the `value` getter with a mask and empty input', async ({page}) => {
		const target = await renderInput(page, {
			mask: '%d-%d'
		});
