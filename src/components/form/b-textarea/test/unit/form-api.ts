...
test.describe('<b-textarea> form API', () => {
	...
	test('resetting the textarea with no default value', async ({page}) => {
		const target = await renderTextarea(page, {
			value: '10'
		});
	...
	test('resetting the textarea with the default value', async ({page}) => {
		const target = await renderTextarea(page, {
			default: '10'
		});
