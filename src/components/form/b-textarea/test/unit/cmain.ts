...
test.describe('<b-textarea>', () => {
	...
	// test('providing `maxLength` and `messageHelpers`', async ({page}) => {
	// 	const target = await renderTextarea(page, {
	// 		maxLength: 20,
	// 		messageHelpers: true
	// 	});
	...
	test('providing `maxLength` and the `limit` slot', async ({page}) => {
		const target = await renderTextarea(page, {maxLength: 20}, {
			limit: ({limit, maxLength}) => {
				console.log(1212, limit, maxLength);
				return `Characters left: ${limit}. The maximum characters is ${maxLength}`
			}
		});

		console.log(await target.evaluate(async ({unsafe}) => {
			const
				scan: unknown[] = [],
				limitEl = unsafe.block!.element('limit')!;
			...
			for (const value of values) {
				unsafe.value = value;
				await unsafe.nextTick();
				scan.push(unsafe.value, limitEl.textContent);
			}
