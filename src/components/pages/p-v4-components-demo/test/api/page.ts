...
export default class DemoPage {
	...
	async goto(query: string = ""): Promise<DemoPage> {
		const root = this.page.locator("#root-component");

		await this.page.goto(
			concatURLs(this.baseUrl, `${this.pageFileName}.html`) +
				(query.length > 0 ? `?${query}` : ""),
			{ waitUntil: "networkidle" }
		);
		await root.waitFor({ state: "attached" });

		this.component = await root.evaluate(
			(ctx) => (<DemoPage>(<unknown>ctx)).component
		);
