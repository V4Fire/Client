...
module.exports = (page) => {
	...
	['historyApiRouterEngine', 'inMemoryRouterEngine'].forEach((engine) => {
		describe(`b-router with engine ${engine} simple using`, () => {
			beforeEach(async () => {
				root = await initRouter(page, engine);
			});

			if (engine === 'inMemoryRouterEngine') {
				generateInMemoryEngineSpecs();

			} else {
				generateHistoryApiEngineSpecs();
			}
			...
			function generateInMemoryEngineSpecs() {
				const
					engine = 'inMemoryRouterEngine';
				...
				it('checking the `route` property without initialRoute', async () => {
					root = await initRouter(page, engine, null);
					expect(await root.evaluate(({route}) => route == null)).toBeTrue();
				});
			...
			function generateHistoryApiEngineSpecs() {
